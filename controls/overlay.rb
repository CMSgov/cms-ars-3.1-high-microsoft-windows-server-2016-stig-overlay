include_controls 'microsoft-windows-server-2016-stig-baseline' do
  ...
  control 'V-73231' do
    title 'Manually managed application account passwords must be changed at least every 60 days and when a system administrator with knowledge of the password leaves the organization.'
  ...
  control 'V-73281' do
    title 'Windows Server 2016 must employ automated mechanisms to determine the state of system components with regard to flaw remediation no less often than once every seventy-two (72) hours.'
  ...
  control 'V-73283' do
    title "Windows Server 2016 must automatically remove or disable temporary user accounts after 30 days."
    ...
    If temporary accounts are used, the operating system must be configured to automatically terminate these types of accounts after a CMS-defined time period of 30 days.
    ...
    If \"AccountExpirationDate\" has not been defined within 30 days for any temporary user account, this is a finding.
    
    Member servers and standalone systems:
    Open \"Command Prompt\".
    Run \"Net user [username]\", where [username] is the name of the temporary user account.
    If \"Account expires\" has not been defined within 30 days for any temporary user account, this is a finding."
    desc "fix", "Configure temporary user accounts to automatically expire within 30 days.
  ...
  control 'V-73285' do
    title "Windows Server 2016 must automatically remove or disable emergency accounts after the crisis is resolved or within 24 hours."
  ...
  control 'V-73309' do
    title "Windows 2016 account lockout duration must be configured to 0."
    ...
    Configuring this to \"0\", requires an administrator to unlock the account."
    desc "fix", "Configure the policy value for Computer Configuration >> Windows Settings >> Security Settings >> Account Policies >> Account Lockout Policy >> \"Account lockout duration\" to \"0\" minutes.
    A value of \"0\" is requires an administrator to unlock the account."
  end

  control 'V-73313' do
    title "Windows Server 2016 must have the period of time before the bad logon counter is reset configured to 120 minutes or greater."
    desc "check", "Verify the effective setting in Local Group Policy Editor.
  ...
  control 'V-73315' do
    title "Windows Server 2016 password history must be configured to 12 passwords remembered."
  ...
  control 'V-73321' do
    title "Windows Server 2016 minimum password length must be configured to 15 characters."
    desc "check", "Verify the effective setting in Local Group Policy Editor.
    Run \"gpedit.msc\".
    Navigate to Local Computer Policy >> Computer Configuration >> Windows Settings >> Security Settings >> Account Policies >> Password Policy.
  ...
  control 'V-73387' do
    title "The directory service must be configured to terminate LDAP-based network connections to the directory server after 30 minutes of inactivity."
    ...
    cn=Windows NT,cn=Services,cn=Configuration,dc=[forest-name]\" -attr LDAPAdminLimits
  ...
  control 'V-73605' do
    ...
    desc "To ensure secure CMS websites and CMS-signed code are properly validated, the system must trust the CMS Root Certificate Authorities (CAs).
    The CMS root certificates will ensure that the trust chain is established for server certificates issued from the CMS CAs."
  ...
  control 'V-73607' do
    title "The CMS Interoperability Root CA cross-certificates must be installed in the Untrusted Certificates Store on unclassified systems."
    ...
    desc "check", "Verify the CMS Interoperability cross-certificates are installed on unclassified systems as Untrusted Certificates."
  ...
  control 'V-73613' do
    title 'Domain Controller PKI certificates must be issued by the CMS PKI or an approved External Certificate Authority (ECA).'
    desc 'check', 'This applies to domain controllers. It is NA for other systems.
    Run "MMC".
  ...
  control 'V-73641' do
    title 'The maximum age for machine account passwords must be configured to 60 days or less.'
